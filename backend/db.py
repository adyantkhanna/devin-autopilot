...
def init_db():
    """Create tables and seed default config if they don't exist."""
    conn = get_conn()
    try:
        conn.executescript("""
...
def get_config(key: str) -> str | None:
    """Return a system config value by key, or None if not set."""
    conn = get_conn()
    try:
...
def set_config(key: str, value: str) -> None:
    """Insert or update a system config key-value pair."""
    conn = get_conn()
    try:
...
def upsert_issue(issue: dict) -> None:
    """Insert a new issue or update an existing one by github_number."""
    conn = get_conn()
    try:
...
def get_issue(github_number: int) -> dict | None:
    """Fetch a single issue by GitHub number."""
    conn = get_conn()
    try:
...
def get_untriaged_issues() -> list[dict]:
    """Return all open issues that have not yet been triaged."""
    conn = get_conn()
    try:
...
def get_all_issues_ranked() -> list[dict]:
    """Return all issues ordered by manual priority then computed priority score."""
    conn = get_conn()
    try:
        rows = conn.execute("""
            SELECT * FROM issues
...
def update_triage(github_number: int, triage: dict) -> None:
    """Store triage results for an issue."""
    conn = get_conn()
    try:
        conn.execute("""
            UPDATE issues SET
...
def update_dispatch(github_number: int, fields: dict) -> None:
    """Update dispatch-related fields for an issue."""
...
def set_manual_priority(github_number: int, user: str) -> None:
    """Move an issue to the top of the manual priority queue."""
    conn = get_conn()
    try:
...
def reorder_issues(ordered_numbers: list, user: str) -> None:
...
def get_active_sessions() -> list[dict]:
    """Return all issues with an active Devin session."""
    conn = get_conn()
    try:
...
def get_pr_open_issues() -> list[dict]:
    """Return all issues that have an open PR awaiting review."""
    conn = get_conn()
    try:
...
def get_next_queued_for_autopilot(limit: int = 2) -> list[dict]:
    """Return the next auto-fixable issues ready for autopilot dispatch."""
    conn = get_conn()
    try:
        rows = conn.execute("""
            SELECT * FROM issues
...
def log_activity(github_number: int | None, event_type: str, message: str, triggered_by: str = "system") -> None:
...
def get_stats(period: str = "7d") -> dict:
    """Compute aggregate stats for the dashboard, filtered by time period."""
...
def get_completed_this_week() -> list[dict]:
    """Return issues completed in the last 7 days."""
    conn = get_conn()
    try:
...
def get_needs_human_issues() -> list[dict]:
    """Return triaged issues that require human review."""
    conn = get_conn()
    try:
